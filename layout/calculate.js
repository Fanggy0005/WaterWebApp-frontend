function calculateSystem(treeLocations) {
    ...
    setTimeout(() => {
        ...
        console.log("Parameters received:", {
            ...
            pipelineRows: window.pipelineRows // Use window.pipelineRows instead
        });
        ...
        // Use window.pipelineRows for validation
        ...
        function cal_PE(Q) {
            ...
            if (Q <= 42) {
                ...
                return "Overdue!"; // ถ้า Q เกินช่วงที่กำหนด
            }
        
            return PE; // PE = ขนาดท่อPE 
        }
        ...
        function loss_PE(Q,PE) {
            ...
            if (PE === 32) {
                ...
                return "Overdue!"; // ถ้า PE ไม่ตรงกับค่าที่กำหนด
            }
        
            return PE_loss; // ค่า loss ของ PE ต่อ10เมตร
        }
        ...
        function cal_lossPE(boxLength, PE_loss) {
            ...
            Branch_loss_PE = (boxLength * PE_loss) / 10;
            
            //console.log("cal_lossPE", Branch_loss_PE);
            return Branch_loss_PE;
        }
        
        console.log("cal_lossPE", cal_lossPE(149,0.44));
        
        // outhole = รูที่น้ำออก นับจากจำนวนต้นทุเรียนในท่อย่อย(PE) หรือ นับจากจำนวนท่อPE(PVC)
        function Edit_loss(outhole) {
            ...
            if (outhole == 1){
                ...
                Edit_pipe_loss = 0.512;
            }else if (outhole == 3) {
                Edit_pipe_loss = 0.434;
            }else if (outhole == 4) {
                Edit_pipe_loss = 0.405;
            }else if (outhole == 5) {
                Edit_pipe_loss = 0.380;
            }else if (outhole > 5 && outhole <= 10) {
                Edit_pipe_loss = 0.365;
            }else if (outhole > 10 && outhole <= 15) {
                Edit_pipe_loss = 0.358;
            }else if (outhole > 15 && outhole <= 20) {
                Edit_pipe_loss = 0.354;
            }else if (outhole > 20 && outhole <= 30) {
                Edit_pipe_loss = 0.350;
            }else if (outhole > 30 && outhole <= 40) {
                Edit_pipe_loss = 0.349;
            }else if (outhole > 40 && outhole <= 50) {
                Edit_pipe_loss = 0.348;
            }else if (outhole > 50) {
                Edit_pipe_loss = 0.347;
            }
        ...
        function cal_PVC(Q_all) {
            ...
            if (Q_all >= 42 && Q_all <= 100) {
                PVC = 1.25;
            }else if (Q_all > 100 && Q_all < 150) {
                PVC = 1.5;
            }else if (Q_all > 150 && Q_all <= 260) {
                PVC = 2;
            }else if (Q_all > 260 && Q_all <= 420) {
                PVC = 2.5;
            }else if (Q_all > 420 && Q_all <= 550) {
                PVC = 3;
            }else if (Q_all > 550 && Q_all <= 750) {
                PVC = 4;
            }
        ...
        function cal_lossPVC(verticalLength, loss_PVC) {
            ...
            Main_loss_PVC = (loss_PVC * verticalLength) / 10;
            console.log("verticalLength and loss_PVC", verticalLength, loss_PVC);
            console.log("cal_loss_PVC", Main_loss_PVC); // แสดงค่า Main_loss_PVC ใน console log
            return Main_loss_PVC;
        }
        //console.log(cal_lossPVC(100,0.06));
        //console.log(Edit_loss(10));
        //console.log(Edit_loss(10)*cal_lossPVC(100,0.06)); // loss ท่อเมน
